...
| D13 | P1 | Data quality | **IndiGo co-brand earn is inconsistently decomposed.** Kotak/SBI/IDFC/HDFC 6E cards model `effective_rate` as the receipt-visible total (19–22 BluChips/₹100), which includes IndiGo's *own* program earn — with no `card_attributable_rate`. Axis IndiGo cards are decomposed (card-side ~3–7). This overstates the non-Axis IndiGo cards' card-side value by ~3–4×. Add `card_attributable_rate` to `kotak-indigo`, `kotak-indigo-xl`, `sbi-indigo`, `idfc-first-indigo`, `hdfc-6e-rewards`, `hdfc-6e-rewards-xl` (D-8 pattern). |

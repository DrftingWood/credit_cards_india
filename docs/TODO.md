...
| E19 | Open | Schema | **No field for a RELATIONSHIP gate.** Added `axis-burgundy-private` 2026-07-28; the scrape made the gap concrete. The card is issued only to Axis Burgundy Private account holders (reported Rs 5 crore Total Relationship Value within 12 months), and its Rs 50,000 fee is waived entirely BY that relationship — not by any spend threshold. `eligibility` has age/income/score/residency and `fee_waiver` has `spend_inr`, so neither the gate nor the waiver is expressible; both survive only as prose. Income is left null rather than guessed. Same defect on `hsbc-premier` (Rs 20,000 waived by a Premier relationship — the single biggest variable in that card's value) and named in prose on `axis-reserve` and `axis-olympus`. Proposal: `eligibility.relationship_required` (issuer programme + minimum relationship value) and a `fee_waiver.relationship` variant. Until then, `status: invite-only` is the only lever that keeps such cards out of open recommendations. Also worth noting the "up to 6% back" claim on this card rests on miles-transfer ratios the T&C explicitly refuses to publish ("Enjoy dynamic conversion ratios"), so unit value is held at the Rs 0.20 floor — same E5/E18 pattern. |

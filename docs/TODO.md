...
five-card stack; the three items below are the modelling gaps that dive exposed.
...
| E3 | Open | Schema | **No spend-slab primitive for RISING tier schedules.** `axis-cashback` pays 2% on the first ₹5,000 of monthly online spend, 5% to ₹40,000, then 7% above — total cap ₹4,000, reached at ₹70,714. The schema can express one rate plus a cap, so the record encodes the TOP marginal slab and documents the table in `notes`/`capping_rules`. Two consequences: (a) spend below ~₹70k is overstated; (b) `portfolio.ts`'s greedy allocator assumes concave (falling) marginal rates and is only indicative for rising schedules — the cap-bound total stays right because the cap binds. Fix needs an `accelerated[].slabs[]` construct (`upto_spend_inr`, `rate`, `max_cashback_inr`). Other likely slab cards: audit `sbi-phonepe-purple`, `sbi-tata-neu-plus`, `hdfc-regalia-gold` (the 4 census pairs where a 2-tranche fit is lossy). |
| E4 | Open | Schema | **Base rate cannot be channel-scoped.** `axis-cashback`'s 0.75% base is paid only on card-present (POS) spend and travel — online spend past the ₹4,000 accelerated cap earns NOTHING. `base` has no channel field, so the calculator applies 0.75% to all overflow and overstates the card. Needs `base.channel` mirroring `accelerated[].channel`, then re-audit every card whose base is narrower than "all spend". |
| E5 | Open | Data quality | **Sweep the 10 accelerators whose rate is an "up to N%" ceiling.** `ratesFlags` now flags a rate whose `notes` quote an "up to N%" equal to the encoded `effective_rate` — the axis-cashback failure mode, where marketing copy's ceiling was stored as a flat rate. Confirmed offenders to re-source from T&C: `axis-cashback` (fixed 2026-07-28), `sbi-irctc-platinum`, `au-paytm`, `idfc-first-hello-cashback`, `idfc-first-wow-black`, `idfc-first-diamond-reserve`, `idfc-first-wow`, `idfc-first-lic-select`. |

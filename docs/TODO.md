...
| D28 | Done | Booking engine | **engine_v2.py red-team flaws R1–R7** — engine-side items complete 2026-07-05. Cap/MCC scoping (D33, c8de69f); channel gating (c937ac6): `channel.required` accelerators count only in the absolute ceiling, never the realistic floor — routing optimism lives with value optimism, layers never blended (replay: realistic travel top is now a plausible 8.6%; Infinia SmartBuy shows in the ceiling column only). Ranking by the realized floor is the documented honest default, not an asymmetry bug. Portal-markup modelling (R1) is `booking-savings/holistic.py` + `site/lib/booking.ts` territory BY DESIGN (net-cost model with sourced markups) — not an engine_v2 defect. |
...
| D34 | P1 | Data quality | **D22 expansion — ungated-accelerator pattern is ~45 cards.** CHUNK 1 DONE 2026-07-05 (54522c0): all 11 D22-scope cards gated (17 accelerators — amex-smartearn, au-ixigo, axis-ace/-airtel/-fibe/-flipkart/-shoppers-stop, icici-mmt, sbi-landmark x2, sbi-reliance-prime) + 28 known.yaml tokens. REMAINING (~34 new-finding cards): fuel co-brands missing fuel-network gates (axis-indianoil-premium, hdfc-indianoil, icici-hpcl x2, idfc-first-power x2, sbi-bpcl/-octane); high-severity ungated (axis-samsung-signature 10% no-cap, sbi-reliance base, hdfc-regalia-gold marquee 25%, hdfc-moneyback-plus 20%); app-ecosystem cards with NO merchants[] to gate on (sbi-paytm x2, sbi-phonepe-purple — need merchants+channel authored from T&C); partner lists (hdfc-freedom/millennia, hsbc-live-plus, rbl-play, sbi-simplyclick x2); ~12 co-brand ecosystems (axis-spicejet-voyage, bob-etihad x2, hdfc-swiggy/-phonepe x2/-shoppers-stop x2/-tata-neu-infinity non-partner rows, icici-mmt-platinum, sbi-landmark base, au-paytm, hdfc-upi-rupay). |

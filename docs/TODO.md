...
| D20 | P1 | Data quality | **Uncapped high-rate accelerators (missing caps).** The decoupled scorer's flag pass exposed implausible uncapped rates: `axis-airtel` (~25%), `sbi-phonepe-select-black` and `hsbc-rupay-cashback` (~10% uncapped). Real cards cap these — add the missing `cap_per_cycle` from issuer T&C. |
| D21 | P2 | Data quality | **Co-brand accelerator not channel-gated.** `sbi-flipkart` earns its elevated online rate on *any* online spend (its accelerator has no `channel`), so it surfaces for an Amazon shopper. Gate co-brand accelerators to their merchant (add a `flipkart` cobrand-merchant channel). Audit other co-brands for the same. |

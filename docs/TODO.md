...
| D17 | P0 | Recommender | **Brand preferences are inert** (F1 in `docs/RECOMMENDER-FLAWS-2026-07.md`). `applyApplicability:true` + zero authored `applicability_pct` makes every co-brand/portal rate collapse to base, so `primary == general` and the wizard's Brand-fit step + `channelMix` do nothing. Fix via authored applicability (D12) or merchant-level spend input. |
...
| D33 | P1 | Booking engine | **engine_v2.py diverges from the site engines** (2026-07-05 E1/E2 audits). (a) `compute()` line 66 ignores `effective_per_inr` — a Magnus-shaped record (35/₹200, base per ₹100) scores 2x rich; (b) `effective_rate: 0` misroutes to the multiplier path via `or`; (c) never uses `card_attributable_rate` → double-counts program earn vs scorer; (d) missing `cap_unit` → uncapped (calculator defaults "points"); (e) base `cap_per_cycle`, `reward_cap`, `mcc_exclusions` unenforced; (f) accelerator selection by raw rate, not cap-aware value. Fix + add a Python test file (none exists). Overlaps R1–R7 (D28). |

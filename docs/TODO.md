...
| D32 | P2 | Tests | **Regression locks for the three historical rate-pipeline bugs.** 2026-07-05 audit: A1 units-as-percent IS locked (calculator.test.ts:601, detail-derivations.test.ts:8); per_inr<=0 now locked for formatAcceleratedRate + ratesFlags too. STILL MISSING: `computeHeadlineRatePct` in site/scripts/build.mjs is unexported/untested (its per_inr guard has no lock), and no three-consumer consistency test asserts build.mjs, calculator, and detail-derivations produce the same pct for one record. Export it (or move next to rate-math.mjs) and add both tests. |
| D33 | P1 | Booking engine | **engine_v2.py diverges from the site engines** (2026-07-05 E1/E2 audits). DONE 2026-07-05 (7bc1617): (a) `effective_per_inr` honoured; (b) `effective_rate: 0` semantics; (c) `card_attributable_rate` preferred (D-8); (d) missing `cap_unit` defaults to points; base floor on selection; first test file added (booking-savings/test_engine_v2.py). REMAINING: (e) base `cap_per_cycle`, `reward_cap`, `mcc_exclusions` unenforced; (f) accelerator selection by raw rate, not cap-aware value. Overlaps R1–R7 (D28). |
...
| D35 | P2 | Tests | **Cap-conversion test gaps** (2026-07-05 E2 audit): no coverage for `miles` cap_unit, `annual` cycle /12, `per-txn` ≈ monthly, points-cap with null unit_value (cap silently unenforced, calculator.ts), `spend-inr` on base/reward_cap returning null (latent authoring trap — also decide whether to support it), `"unlimited"` string through the typeof check, and base cap in points units. Shared-cap-across-buckets and over-cap-base-fallback were locked 2026-07-05. |
